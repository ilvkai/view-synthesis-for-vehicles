...
class PoseHMDataset(UGANDataset):
    ...
    def compute_pose_map_batch(self, pair_df, direction):
        ...
        batch = np.empty([self._batch_size] + list(self._image_size) + [20])
        i = 0
    ...
    def compute_cord_warp_batch(self, pair_df):
        ...
        for _, p in pair_df.iterrows():
            ...
            if self._warp_skip == 'mask':
                batch[0][i] = pose_transform.perspective_transforms(kp_array1, kp_array2)
    ...
    def load_image_batch(self, pair_df, direction='from'):
        ...
        for _, p in pair_df.iterrows():
            if os.path.exists(os.path.join(self._images_dir_train, p[direction])):
                batch[i] = resize(imread(os.path.join(self._images_dir_train, p[direction])), (256,256))
            else:
                batch[i] = resize(imread(os.path.join(self._images_dir_test, p[direction])), (256,256))
            i += 1
