...
from pose_transform import PerspectiveTransformLayer
...
def decoder(skips, nfilters=(512, 512, 512, 256, 128, 3)):
    ...
    for i, (skip, nf) in enumerate(zip(skips, nfilters)):
        if 0 < i < 3:
            ...
            out = block(skip, nf, down=False, leaky=False, dropout=False)
        elif i == len(nfilters) - 1:
            out = Concatenate(axis=-1)([out, skip])
            out = Activation('relu') (out)
...
def concatenate_skips(skips_app, skips_pose, warp, image_size, warp_agg, warp_skip,debug):
    skips = []
    for i, (sk_app, sk_pose) in enumerate(zip(skips_app, skips_pose)):
        if i < 4:
            out = PerspectiveTransformLayer(10 if warp_skip == 'mask' else 1, warp_agg, image_size,debug)([sk_app] + warp)
...
def make_generator(image_size, use_input_pose, warp_skip, disc_type, warp_agg,debug=False):
    # input is 256 x 256 x nc
    use_warp_skip = warp_skip != 'none'
    input_img = Input(list(image_size) + [3])
    output_pose = Input(list(image_size) + [20])
    ...
    if use_input_pose:
        input_pose = [Input(list(image_size) + [20])]
    else:
        input_pose = []
    ...
    if use_warp_skip:
        ...
        enc_layers = concatenate_skips(enc_app_layers, enc_tg_layers, warp, image_size, warp_agg, warp_skip,debug)
    else:
...
def make_discriminator(image_size, use_input_pose, warp_skip, disc_type, warp_agg):
    ...
    if disc_type == 'call':
        ...
        out_inp = PerspectiveTransformLayer(10, warp_agg, image_size) ([out_inp] + warp)
...
class CGAN(GAN):
    def __init__(self, generator, discriminator, l1_penalty_weight, perceptual_penalty_weight, gan_penalty_weight, use_input_pose, image_size,
        ...
        self.__perceptual_penalty_weight = perceptual_penalty_weight
    ...
    def _compile_generator_loss(self):
        ...
        if self._content_loss_layer != 'none':
            ...
            perceptual_loss = K.constant(0)
            if type(reference) != list:
                reference = [reference]
                target = [target]
            for a, b in zip(reference, target):
                perceptual_loss = perceptual_loss + self.__perceptual_penalty_weight * st_loss(a, b)
        if self._l1_penalty_weight != 'none':
        ...
        if self._lstruct_penalty_weight != 0:
            target_struct = self._pose_estimator(self._generator_input[image_index][..., ::-1] / 2)[1][..., :20]
            struct = self._pose_estimator(self._discriminator_fake_input[image_index][..., ::-1] / 2)[1][..., :20]
        ...
        def generator_loss(y_true, y_pred):
            return gan_loss_fn(y_true, y_pred) + l1_loss_fn(y_true, y_pred) + perceptual_loss_fn(y_true, y_pred) + tv_loss(y_true, y_pred) + struct_loss_fn(y_true, y_pred)
        return generator_loss, [gan_loss_fn, l1_loss_fn, perceptual_loss_fn, tv_loss, struct_loss_fn]
